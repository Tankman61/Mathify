...
cred = credentials.Certificate("mathifyignition-firebase-adminsdk-t3rvl-1bc71aa5f2.json")
firebase_admin.initialize_app(cred, {
    'databaseURL': 'https://mathifyignition-default-rtdb.firebaseio.com/'
})
...
def login():
    ...
    try:
        userdata = db.reference('users/'+username).get()
        if userdata:
